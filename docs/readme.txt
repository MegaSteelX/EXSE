...
textsize：指定文本的相对大小，类似于字号。取值应当为非负整数；当此key未被定义时默认取值10。
color：指定文本颜色。格式为#??????，其中?为从0到F的十六进制数字，??????构成一个RGB色标；未定义时，默认取值#000000（黑色）。
textstyle：定义段落文本的特殊效果，取值为由“N/I/B/S/U”中任意一个或多个字符组成的字符串。其中N表示先清除已存在的效果（N总是最先应用，然后被任何可能存在的效果覆盖），I表示斜体，B表示粗体，S表示删除线，U表示下划线。注意：I和U使用“伪”斜体和粗体；若要使用已存在的斜体或粗体字体，应在fonts文件夹中改变定义。
